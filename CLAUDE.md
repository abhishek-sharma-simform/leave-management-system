...
- `GET /api/v1/leave-requests/team-on-leave?startDate=&endDate=` (auth required, self-service — no role restriction) — `findTeammatesOnLeave` in `leaveRequest.service.ts` looks up the caller's `managerId`, then returns other users sharing that manager whose `APPROVED` or `PENDING` request overlaps the given range (`startDate <= endDate` enforced by `teamOnLeaveQuerySchema`), shaped as `{ startDate, endDate, teammatesOnLeave: [{ userId, name, email, leaveType, startDate, endDate, status }] }`. Meant to back a leave-request form's date picker (check teammates' overlap before submitting) — same overlap semantics as `findOverlappingRequests` (`APPROVED` + `PENDING`, same-manager teammates only) but keyed off a raw date range instead of an existing request id. A caller with no manager gets `{ startDate, endDate, teammatesOnLeave: [] }`, not an error.
