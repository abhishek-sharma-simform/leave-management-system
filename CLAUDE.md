...
- **middleware/auth.middleware.ts** verifies the JWT and attaches `req.user = { id, role }`. **middleware/role.middleware.ts**'s `requireRole(role)` must run after `authMiddleware` and checks `req.user.role` against the Prisma `Role` enum value. `types/express.d.ts` augments Express's `Request.user` type globally — update it if the JWT payload shape changes. `req.log`/`req.id` are augmented globally by `pino-http`'s own type declarations (no local `.d.ts` needed) once `middleware/requestLogger.middleware.ts` is imported anywhere in the compiled program.
- **config/env.ts** loads and validates env vars — `DATABASE_URL` is validated at startup; `JWT_SECRET` currently is not (see Known gaps). Also exports `NODE_ENV` (default `development`) and `LOG_LEVEL` (default `debug`/`info` by env), and loads `.env.test` instead of `.env` when `NODE_ENV === "test"` (Vitest sets this automatically).
- **config/logger.ts** exports the base `pino` `logger` (pretty-printed in non-production, raw NDJSON in production). **middleware/requestLogger.middleware.ts** wraps it with `pino-http`, generating a `crypto.randomUUID()` per request (echoed as `X-Request-Id`) and mounting first in `app.ts` (before `express.json()`) so `req.log` exists even for JSON-parse failures. All catch-block/error logging uses `req.log.error({ err }, "...")`, not `console.*`.
...
`console.*` is not used anywhere in `src/` — all logging goes through `req.log` (a `pino-http`-bound child logger carrying that request's correlation id) in controllers/middleware, or the base `logger` from `config/logger.ts` for non-request contexts (`server.ts`'s startup line). Don't reintroduce `console.*` in new code.
